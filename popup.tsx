...
function IndexPopup() {
  ...
  const fetchData=async()=>{
      try {
          ...
          console.log(ip);
          const res_location=await axios.get(`http://ipinfo.io/${ip}/json?token=${apiKey}`)
  ...
  return (
    <div className="bg-blue-200 w-[400px] h-[400px] flex flex-col items-center justify-center border-4 border-double border-blue-600 ">
      {country&&city && <div className="text-2xl font-sans flex flex-col gap-3 font-bold text-center mb-5 text-black uppercase">
        <div>{country}</div>
    ...
    {isLoading && 
      <div>
        <img src="https://cdn.discordapp.com/attachments/938278149622472714/1146801792508756049/18-location-pin.png" alt="logo" height={200} width={180} className="animate-bounce"/>
      </div>}
      {!isLoading&&<button
        onClick={fetchData}
        disabled={isLoading}
